...
func TestExecute(t *testing.T) {
	scenarios := []struct {
		...
		{
			desc: "context was initialized using WithCache but cancelled",
			test: func(t *testing.T) {
				var evaled int32 = 0
				...
				ctxWithCancel, cancelFn := context.WithCancel(ctxWithCache)
				cancelFn()
				...
				for i := 0; i < 100; i++ {
					...
					go func() {
						...
						outcome, extra := Execute(ctxWithCancel, "executionKey", memoizedFn)
						assert.Equal(t, 0, outcome.Value)
						assert.Equal(t, context.Canceled, outcome.Err)
						assert.True(t, extra.IsMemoized)
						assert.False(t, extra.IsExecuted)
					}()
				}
...
func TestNewTypedOutcome(t *testing.T) {
	scenarios := []struct {
		...
		{
			desc: "outcome Value is not of expected type",
			test: func(t *testing.T) {
				o := Outcome{
					Value: "string",
				}
		...
		{
			desc: "outcome Value is of expected type",
			test: func(t *testing.T) {
				o := Outcome{
					Value: 1,
				}
